...
class ActorCriticPolicy(BasePolicy):
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False, param_noise: bool = False,sigma: float = 0.1) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        ...
        """
        # param_noise = True
        #for some reason if i change param_noise from here i get low rewards



        if param_noise:
            latent_pi, latent_vf, latent_sde = self._get_latent(obs,param_noise,sigma)

        else:
            latent_pi, latent_vf, latent_sde = self._get_latent_unnoisy(obs, param_noise, sigma)
    ...
    def _get_latent(self, obs: th.Tensor, param_noise: bool = False, sigma: float = 0.1) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        ...
        latent_pi, latent_vf = self.mlp_extractor(features,param_noise,sigma)
    ...
    def _get_latent_unnoisy(self, obs: th.Tensor, param_noise: bool = False, sigma: float = 0.1) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        Get the latent code (i.e., activations of the last layer of each network)
        for the different networks.
    ...
    def evaluate_actions(self, obs: th.Tensor, actions: th.Tensor, param_noise: bool=False,sigma: float = 0.1) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        ...
        if param_noise:
            latent_pi, latent_vf, latent_sde = self._get_latent(obs,sigma=sigma)
        else:
            latent_pi, latent_vf, latent_sde = self._get_latent_unnoisy(obs)
        distribution = self._get_action_dist_from_latent(latent_pi, latent_sde)
        log_prob = distribution.log_prob(actions)
        values = self.value_net(latent_vf)
